        ...
        if 'image' in request.files:
            ...
            if image.filename != '':
                try:
                    ...
                    query_db("INSERT INTO images (username, file_name) VALUES (?,?)",  (session['username'], image.filename), True)
                    query_db("UPDATE users SET pics = pics + 1 WHERE username=?", (session['username'],), True)
...
def validate_lib_update_profile_picture(filename):
    ...
    query_db("UPDATE users SET profile_pic=? WHERE username=?", (filename, session['username']), True)
